...
for unit in $GATE_UNITS; do
    ...
    # R3 C-2: пустой снимок ДО — это НЕ «непроверяемое».
    # Замер на проде (2026-07-21): InvocationID пуст РОВНО у тех юнитов, которые
    # с загрузки НИ РАЗУ не активировались (apport-autoreport.path, tpm-udev.path).
    # Юнит, который отработал и остановился, ID сохраняет (dmesg.service,
    # e2scrub_reap.service — inactive, но ID непустой). Отсюда обе ветки верны:
    #   • pre пуст  → юнит с загрузки не запускался; сейчас active ⇒ активация была;
    #   • pre есть  → сравниваем ID (ниже), смена ⇒ активация была.
    # Связка «до — не работал, сейчас — active с непустым ID» есть ПОЛОЖИТЕЛЬНОЕ
    # доказательство свежей активации, самое сильное из возможных.

...
ПОСЛЕ backfill (0071) И выкатки кода, где save() резолвит Common (порядок деплоя план #262:
...
revision = "20260630_0072"
down_revision = "20260630_0071"
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    bind = op.get_bind()
    # Префлайт (ревью R2): если backfill 0071 не прогнан ИЛИ код (save резолвит Common) не выкачен — остались
    ...
    if remaining:
        raise RuntimeError(
            f"#262 0072: {remaining} строк assistant_memories с category_id IS NULL — сначала прогони backfill "
            "0071 и убедись, что код (save() резолвит Common) выкачен и перезапущен. SET NOT NULL прерван."
        )

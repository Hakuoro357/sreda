...
⚠️ ПОРЯДОК ДЕПЛОЯ (субагент R1 MAJOR): рестарт 2b-кода ДОЛЖЕН идти ДО этой миграции —
``pull → safe_restart (2b) → alembic upgrade head → verify``, НЕ наоборот. Причина: прод-код 2а
пишет normalized_title_hash на ЛЕГАСИ/композит-путях и НЕ имеет reuse-backstop. Если индекс
появится, пока крутится 2а-код, легитимное повторное legacy-создание дубля (дательная задача с тем
же title+date) словит необработанный IntegrityError. 2b-код пишет hash ТОЛЬКО на ReAct-пути (легаси
→ NULL → вне индекса) и имеет backstop, поэтому он безопасен и БЕЗ индекса (pre-check дедупит) — значит
рестарт 2b до миграции окна не создаёт. (Исключение из обычного «migrate→restart»: код forward-совместим
без нового индекса, а СТАРЫЙ код с индексом — нет.)

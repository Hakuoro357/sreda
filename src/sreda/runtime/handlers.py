...
- НЕ говори "Готово" / "Сделала" / "Настроила" / "Запомнила" пока в этом ЖЕ turn'е не было реального tool-call'а который это выполнил. Если собираешься выполнить действие — сначала вызов tool'а, ПОТОМ рапорт. Пустая фраза "Готово" без соответствующего tool-call'а — запрещена.

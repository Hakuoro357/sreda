...
- Меню на неделю:
    ...
    * Формат ответа для меню на неделю: заголовок `Меню на неделю: 25–31 мая`; дальше каждый день отдельным блоком, в заголовке дня всегда указывай день недели и дату: `Понедельник, 25 мая` / `Вторник, 26 мая`; ниже строки `Завтрак: ...`, `Обед: ...`, `Ужин: ...`. Между днями оставляй пустую строку. Не склеивай ужин одного дня с названием следующего дня. Не используй markdown-таблицы — в Telegram/MAX они читаются плохо. Финальный вопрос (`Хочешь, соберу список покупок?`) отделяй пустой строкой.
...
_MENU_FOLLOWUP_RE = re.compile(
    r"(\n[—-]\s*Ужин:[^\n]+?)\s+"
    r"(?=(?:Хочешь|Если хочешь|Могу|Нужно|Собрать)\b)"
)

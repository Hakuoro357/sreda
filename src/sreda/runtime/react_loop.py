...
def _system_prompt(today_str: str) -> str:
    return (
        ...
        "ПОЛЬЗОВАТЕЛЬ; не назвал (или расплывчато «утром») → спроси «во сколько?», не выдумывай "
        "(цикл сам не даст создать без названного времени).\n"
        ...
        "«утром/вечером») — спроси «во сколько напомнить?», НЕ ставь на угаданное время (цикл "
        "сам не даст создать напоминание без названного времени). Подтверждая напоминание/"
...
# #180: КОНКРЕТНОЕ время в словах пользователя. Расплывчатое «утром/вечером» НЕ считается —
# это скрытый дефолт (8/9/10), а принцип Бориса: не хватает точности — спросить, не додумывать.
# ВАЖНО (Codex/субагент R2): «в N» считаем временем ТОЛЬКО при подтверждающем контексте — час-
# слово, конец фразы или глагол-инфинитив следом — иначе ловим количество («в 5 человек», «в 2
# магазинах»). Перекос в ложный ПРОПУСК (лишний вопрос) безопасен; ложное срабатывание (молчаливый
# дефолт) — нет. Слова-числа («в девять») намеренно НЕ ловим (риск «в семью»/«в двух словах»).
# Частые глаголы-напоминания (ТОЧНЫЕ инфинитивы, не stem) — generic «-ть/-чь» ловил женские
# существительные («часть/область/сеть/новость/ночь»), Codex R3-R4. Пропуск редкого глагола →
# лишний вопрос (безопасно); существительные больше не проходят.
_REMINDER_VERBS = (
    r"позвонить|перезвонить|позвать|забрать|встретить|купить|докупить|сходить|зайти|заехать|"
    r"съездить|полить|принять|выпить|поздравить|оплатить|заплатить|отправить|написать|"
    r"проверить|разбудить|приготовить|помыть|постирать|убрать|покормить|выгулять|погладить|"
    r"сделать|поесть|позавтракать|пообедать|поужинать|собрать|сдать|отвезти|отвести|включить|"
    r"выключить|закрыть|открыть|записаться|встать|лечь|выехать|выйти|заказать|поменять|сменить|"
    r"продлить|вынести|помолиться|выпить")
_TIME_RE = re.compile(
    r"\d{1,2}\s*:\s*\d{2}"                                       # 14:00 (только «:»; «9.05» = дата)
    r"|через\s+\d+\s*(?:мин(?:ут\w*)?|час(?:а|ов)?)\b"          # через 10 минут / через 2 часа
    r"|\b(?:полдень|полночь)\b"                                 # полдень / полночь (точные; НЕ «полночи»)
    # «в/к N» как ВРЕМЯ, если за числом: конец фразы, час-слово, либо ГЛАГОЛ из списка:
    r"|\b[вк]\s*\d{1,2}\b(?=\s*$"
    r"|\s+(?:час(?:а|ов)?\b|ч\b|утра\b|вечера\b|дня\b|ночи\b)"
    r"|\s+(?:" + _REMINDER_VERBS + r")\b)",
    re.IGNORECASE)


def _text_mentions_time(text: str) -> bool:
    """#180: есть ли в тексте КОНКРЕТНОЕ время. True → можно ставить напоминание; False → надо
    спросить «во сколько?» (детерминированно по словам юзера, НЕ по флагу модели)."""
...
def build_slice_tools(session: Any, tenant_id: str, user_id: str) -> list:
    ...
    def schedule_reminder(title: str, trigger_iso: str) -> str:
        ...
        если в запросе времени нет (или лишь расплывчато «утром») — спроси «во сколько?», своё
        НЕ подставляй (цикл сам не даст создать напоминание без названного времени)."""
        # #180: жёсткий гейт «есть ли время» — детерминированно в run_tools по словам юзера
        # (не доверяем аргументу модели). Сюда доходим, только если время уже названо.
        try:
...
def _build_graph(llm: Any, all_tools: list, *,
    ...
    def run_tools(state: ReactState):
        ...
        last_human = _last_human_text(state["messages"])  # #180: для гейта времени напоминания
        ...
        for tc in state["messages"][-1].tool_calls:
            ...
            if tool_obj is None:
                ...
                continue
            # #180 (ключевой принцип, ДЕТЕРМИНИРОВАННЫЙ hard-gate): напоминание без КОНКРЕТНОГО
            # времени в словах пользователя НЕ создаём — просим спросить «во сколько?». Проверка
            # по тексту юзера, а НЕ по флагу/аргументу модели (его модель могла бы выдумать, как
            # и сам момент 09:00). Время названо в одном из сообщений хода → доедет в last_human.
            if name == "schedule_reminder" and not _text_mentions_time(last_human):
                out.append(ToolMessage(
                    content=("need_time: пользователь не назвал КОНКРЕТНОЕ время. Спроси одним "
                             "вопросом «во сколько напомнить?» и НЕ создавай, пока не ответит. "
                             "Расплывчатое «утром/вечером» — уточни час, не подставляй сама."),
                    name=name, tool_call_id=tc["id"]))
                continue

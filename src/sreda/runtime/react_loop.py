...
_REACT_TOOL_DESC: dict[str, str] = {
    ...
    "search_recipes": (
        ...
        "мой рецепт X» и в начале plan_week_menu (переиспользовать сохранённые). Это книга, НЕ "
        "меню на день (для плана дня — list_menu). Read-only."
    ),
    "get_recipe": (
    ...
    "recall_memory": (
        ...
        "X / что у меня про Y». query — фраза/ключевые слова; top_k 1-10 (для списков 10). "
        "Возвращает JSON [{content, source, score, metadata}]."
    ),
    "save_episode": (

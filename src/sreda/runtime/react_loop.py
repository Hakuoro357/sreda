...
def _prev_turn_families(messages: Any) -> tuple[str, ...]:
    ...
    булка» должно видеть тот же раздел. НЕ считаются (#352 R1): отклонённый
    кандидат-confirm (структурный маркер _CONFIRM_DECLINED_TEXT - инструмент НЕ
    исполнялся, R1 terra) и web (R1 субагент: web - baseline, не own-data контекст;
    зеркало _prev_open_domains - иначе «погода + список» ходом раньше подсовывает
    классификатору web-дистрактор и заново ломает кейс #352). Фильтр по
    _USER_DOMAINS: семьи вне enum классификатора (onboarding/ui/utility) в хинт не
    попадают - классификатор таких слов не знает. Пусто = прошлого контекста нет
    ...
    for m in reversed(msgs[:-1]):
        ...
        if isinstance(m, ToolMessage) and getattr(m, "name", None):
            if str(getattr(m, "content", "")) == _CONFIRM_DECLINED_TEXT:
                continue  # отклонённый кандидат: раздел юзер явно НЕ тронул
            _art = getattr(m, "artifact", None)

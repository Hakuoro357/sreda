...
# Разделитель — ЛЮБАЯ серия бэктиков, а не только 1 или 3 (R5 sol: у «``foo **bar** baz``»
# двойные бэктики читались как два пустых inline-span'а, и содержимое между ними попадало
# под нормализацию). Закрывающим считается серия ТОЙ ЖЕ длины. Незакрытые формы тоже
# защищаем (R4 terra: модель нередко обрывает вывод): ``` и длиннее — до конца текста,
# 1-2 бэктика — до конца строки. Внутри каждой альтернативы ЗАКРЫТАЯ форма идёт первой.
_CODE_SPAN_RE = re.compile(
    r"(?P<f>`{3,})(?:.*?(?P=f)|.*)"                 # блок: закрытый ИЛИ до конца текста
    r"|(?P<i>`{1,2})(?:[^\n]*?(?P=i)|[^\n]*)",      # inline: закрытый ИЛИ до конца строки
    re.DOTALL,
)

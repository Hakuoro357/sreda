...
# не начинается и не кончается пробелом. Это оставляет как есть арифметику «2**3 + 4**5» и
# глоб-маски в кавычках («"**/*.py"»). Без DOTALL — жирный заголовок не переносится через строку.
_MD_BOLD_RICH_RE = re.compile(r"(?<![^\s([{«])\*\*(?=\S)(.+?)(?<=\S)\*\*(?![^\s)\]}»:;,.!?])")
...
def _sub_outside_code(pattern: re.Pattern[str], repl: str, text: str) -> str:
    """Применить подстановку ко всему тексту, КРОМЕ кода в бэктиках."""
    out: list[str] = []
    last = 0

...
_CONFIRM_PHRASE = {
    ...
    # #409: СТАТИЧНАЯ фраза (см. докстринг _confirm_phrase) — «очистить всё» честно про всё;
    # динамическая с именами соврала бы про объём, перечислив лишь часть позиций.
    "clear_shopping_list": "очищу весь список покупок",
...
def _confirm_phrase(name: str, session: Any, tenant_id: str, user_id: str) -> Any:
    ...
    (clear_menu/clear_bought_shopping/clear_shopping_list #409) остаются статичными — это честно
    про всё (перечислять имена там значило бы соврать про объём). Иначе — статичный
...
_REACT_TOOL_DESC: dict[str, str] = {
    ...
    "remove_shopping_items": (
        ...
        "#409: для ОТДЕЛЬНЫХ позиций; очистить ВЕСЬ список — clear_shopping_list (без id)."
    ),
    ...
    "clear_bought_shopping": (
        ...
        "за покупками закончен. Без аргументов. Очистить ВЕСЬ список (текущие покупки) — "
        "clear_shopping_list, не этот."
    ),
    # #409. ЭТО описание видит Фредди (ToolSpec.description кормит задепрекейченный планировщик).
    # Прод-замер: «очисти список покупок» срывалась в 2 прогонах из 5 — модель шла в list_shopping
    # за id и сдавалась («не могу очистить одним действием» / переспрос). Потому формулировка
    # снимает ровно эту развилку: единственный способ + id НЕ нужны + не зови list_shopping.
    "clear_shopping_list": (
        "Очистить ВЕСЬ список покупок целиком — все текущие (не купленные) позиции разом. "
        "Когда: «очисти список покупок», «удали все покупки», «очисти весь список», «убери всё "
        "из покупок». ЕДИНСТВЕННЫЙ способ очистить список полностью. Без аргументов: id НЕ "
        "нужны, list_shopping перед этим звать НЕ надо. Отдельные позиции — "
        "remove_shopping_items по id. Купленное (история) не трогает — для него "
        "clear_bought_shopping. Возвращает, сколько позиций убрано (0 = список уже был пуст)."
    ),
    # рецепты
    "save_recipe": (

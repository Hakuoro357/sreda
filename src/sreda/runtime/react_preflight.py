...
_HINT_RECURRENCE = (
    ...
    "передай recurrence_rule (RFC-5545): «каждый час» → FREQ=HOURLY, «каждый день в 9» → "
    "FREQ=DAILY, «каждую пятницу в 16:00» → FREQ=WEEKLY;BYDAY=FR. trigger_iso = ПЕРВОЕ "
    "срабатывание. НЕ отказывай «могу только однократное» и НЕ ставь молча однократное — "
    "повторы поддерживаются. Пользователь НЕ назвал, до какого времени или сколько раз "
    "повторять — ОБЯЗАТЕЛЬНО уточни ОДНИМ вопросом («до какого времени повторять или "
    "сколько раз?»), затем ставь с recurrence_rule: N раз → добавь ;COUNT=N, до времени → "
    "добавь ;UNTIL=<UTC basic format, напр. 20260710T205959Z>. Конец назван сразу — ставь "
    "без уточнения. (Решение владельца 2026-07-10, #333.)")

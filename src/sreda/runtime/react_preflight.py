...
# #374: директива РАЗЛИЧАЕТ «конкретный список по имени» (show_checklist) от «обзор всех / раздел
# без имени» (list_checklists). Прежний текст велел list_checklists ВСЕГДА → на «покажи список X»
# бот вываливал обзор всех вместо пунктов X (~50% промахов, прод 2026-07-14). Симметрично
# _HINT_CHECKLIST_UNIFIED, но legacy-инструментами. Обзорные слова-разделы («дела», «список дел»,
# «мои списки») ЯВНО помечены как обзор — иначе модель приняла бы «дел» за имя (R2 sol/субагент N5).
# Покупки оговорены как отдельный list_shopping: на «список покупок» роутер и так уводит в shopping
# (_PHRASES), но на легаси/section-пути оговорка не даёт спутать их с чек-листом (R2 sol collision).
# Shopping-директиву в _DIRECTIVE НЕ добавляем: она инжектилась бы и на write/compound/cross/ON-ходы
# покупок и глушила бы их (R2 sol/terra/субагент N1-N4) — покупки разводит few-shot + эта оговорка.
_HINT_CHECKLIST = ("ВАЖНО: пользователь спрашивает про СПИСКИ ДЕЛ (раздел «Дела» = чек-листы со списками "
                   "пунктов). Назван КОНКРЕТНЫЙ список по имени (напр. «список кино», «Поход») → "
                   "show_checklist(list_id_or_title с этим именем): пункты ИМЕННО его, не обзор. "
                   "Слово-раздел БЕЗ имени («дела», «список дел», «мои списки», «какие списки», "
                   "«покажи все») → list_checklists (обзор со счётчиками). Покупки идут ОТДЕЛЬНО в "
                   "list_shopping, это не чек-лист. НЕ показывай напоминания (list_reminders) и НЕ "
                   "показывай задачи (list_tasks).")
...
# ───────────────────────── #333: повторяющиеся напоминания ─────────────────────────
# Прод-инцидент 2026-07-10 (user_tg_755682022): «поставь напоминание с 12:30 каждый час»
# → Фредди отказал «могу только однократное» при НОЛЕ tool calls, хотя schedule_reminder
# умеет recurrence_rule (RFC-5545). Probe ×5: 0 вызовов с FREQ=HOURLY. Класс #279 (ложный
# отказ о своих возможностях). Урок #180: промпт быстрой модели не держит → детерминированный
# хинт по коду (паттерн #215). Сигнал УЗКИЙ (анти-FP): «кажд»-корень И напоминание-корень
# в одном ходе; «добавь молоко в каждый список» (без «напомн») хинт не трогает.
_RECUR_ROOTS = ("кажд",)  # каждый/каждую/каждое/каждые/каждым
_HINT_RECURRENCE = (
    "ВАЖНО: пользователь просит ПОВТОРЯЮЩЕЕСЯ напоминание. schedule_reminder УМЕЕТ повторы — "
    "ВЫЗЫВАЙ его сразу, передай recurrence_rule (RFC-5545): «каждый час» → FREQ=HOURLY, "
    "«каждый день в 9» → FREQ=DAILY, «каждую пятницу в 16:00» → FREQ=WEEKLY;BYDAY=FR. "
    "trigger_iso = ПЕРВОЕ срабатывание. НЕ отказывай «могу только однократное» и НЕ ставь "
    "молча однократное — повторы поддерживаются. Конец серии назван — добавь ;COUNT=N или "
    ";UNTIL=<UTC basic format, напр. 20260710T205959Z>; не назван — всё равно ВЫЗЫВАЙ "
    "инструмент, он сам подскажет спросить. (#333/#350: уточнение конца — механизм, не "
    "твоя инициатива; НЕ спрашивай ДО вызова.)")


def _recurrence_hint(text: str | None) -> str | None:
    """#333: «кажд»-корень + напоминание-сигнал → директива про recurrence_rule, иначе None.
    Детерминированный (не LLM), чистый от state — тестируем прямым вызовом."""
    ...
    if has_root(_RECUR_ROOTS) and has_root(_SEC_REMINDER_ROOTS):
        return _HINT_RECURRENCE
    return None
...
_CQ_WRITE_RE = re.compile(
    ...
    # #376 L2-R1 (sol MAJOR): «внеси/впиши/зафиксируй в список кино» давали items/high →
    # write-shaped ход попадал под сужение (детектор обязан вернуть None на записи).
    r"пометь|галочк|внес|впиш|зафиксир)")
...
# #376 L2-R1 (terra MAJOR): окно {0,2}→{0,4} — «какие ЕЩЁ У МЕНЯ списки» (3 слова)
# не ловилось → items вместо overview → сужение прятало явно запрошенный обзор.
_CQ_OVERVIEW_RE = re.compile(
    r"(какие\s+(?:\w+\s+){0,4}(списки|планы|чек-?листы|дела)"
    r"|все\s+(?:\w+\s+){0,4}(списки|планы|чек-?листы)"
    r"|сколько\s+(?:\w+\s+){0,4}(списков|планов|чек-?листов))")
...
def route_domains(text: str) -> RouteResult:
    ...
    for ct in clauses:
        ...
        # #308 (прод-деградация 2026-07-04): «(глагол) в список X» — «в список» это TARGET списка
        # (положить в чек-лист/покупки), глагол памяти «запиши/сохрани» НЕ должен перебивать его в
        # memory. Убираем memory-претендента, когда клауза кладёт «в список» → остаётся checklists
        # (или shopping для «в список покупок» через форсер). «запиши что кофе» (нет «в список») и
        # «запомни список покупок» (нет предлога «в») не затронуты. НЕ трогаем при отсутствии др.
        # доменов (была только memory → оставляем, иначе клауза осиротеет).
        if "в список" in ct and "memory" in matched and len(matched) > 1:
            matched = {d: n for d, n in matched.items() if d != "memory"}
...
def chat_fact_system_prompt(today_str: str, persona_overlay: str = "",
    ...
    `rich_format=True` (канарейка разметки, только Telegram) — короткое правило вёрстки, чтобы
    разговорные ответы у канарейки выглядели так же, как задачные, И чтобы модель не сыпала
    случайными `*`/`_` в текст, который теперь уходит с parse_mode. Дефолт False → строка
    промпта байт-в-байт прежняя (R2 sol+terra MINOR)."""
    _ov = (persona_overlay or "").strip()
    _fmt = (
        "\n- Формат: если ответ просится списком — заголовок раздела ОДИНАРНЫМИ звёздочками "
        "(*вот так*, двойные ** Telegram не понимает) и с уместным эмодзи, пункты с новой "
        "строки. Одиночные * и _ в обычном тексте не оставляй — на них ломается разметка."
    ) if rich_format else ""
    return (
        ...
        # #298: пустой today_str (SREDA_REACT_TIME_IN_TAIL=ON) → даты нет (стабильный текст),
        # дата+время приходят эфемерным хвостом; непустой (легаси) — как раньше.
        + ("Текущие дата и время указаны в конце последнего сообщения пользователя.\n"
           if not today_str else f"Сегодня {today_str}.\n")
        + "Правила:\n"
        "- Отвечай по-русски, по-доброму и живо, как близкий собеседник — не сухо.\n"
        "- Прямо в этом ответе под рукой только веб: web_search (поиск), fetch_url "
        "(открыть страницу), get_weather (погода). Это про ТЕКУЩИЙ ход, а не про твои "
        "умения вообще; других инструментов здесь не зови.\n"
        "- Среда умеет и напоминания, и задачи, и списки, и память. НИКОГДА не говори, "
        "что «не умеешь» их, и не извиняйся за «несделанное»: способность есть. Но и не "
        "утверждай, что уже сделала или сделаешь это прямо в текущем ответе. Если человек "
        "хочет что-то напомнить, записать или запомнить, не отказывай, а коротко уточни "
        "недостающее: для напоминания что и когда, для задачи или списка что и куда, для "
        "памяти что запомнить.\n"

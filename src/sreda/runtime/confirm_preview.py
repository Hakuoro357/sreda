...
# стемы дней недели для верификатора (ловят ЛЮБОЙ падеж: «среда»/«в среду»/«по средам»)
...
def verify_confirm_text(text: str, facts: ConfirmFacts, *, now: datetime) -> bool:
    ...
    for rel in ("сегодня", "завтра", "послезавтра"):
        if _phrase_in(rel, lower_rest) and rel not in days:
            return False
    # R2/R3 medium: дни недели вне допустимых - ЛЮБАЯ падежная форма («в среду»,
    # «завтра - среда», «по средам»), стемами; каждый найденный день обязан
    # совпадать с днём недели факта
    _wd_fact = facts.when_local.weekday()
    for _stem, _idx in _WEEKDAY_STEMS:
        if re.search(rf"(?<![а-яёa-z]){_stem}", lower_rest) and _idx != _wd_fact:
            return False

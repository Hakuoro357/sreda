...
_SHOPPING_LIST_SHOW = (
    ...
    # #123-добив: человеческий ярлык категории (внутренние коды с «_»
...
# #123-добив: человеческие названия категорий покупок для заголовков групп.
# Внутренние коды таксономии (канон: db/models/housewife_food.py::
# SHOPPING_CATEGORIES — закреплено пин-тестом) → русские ярлыки; неизвестное (пользовательские
# категории) — подчёркивания в пробелы + первая буква вверх, хвост не трогаем
# (агент-ревьюер #123: «СВЧ Товары» не калечить). ⚠ ДУБЛЬ ТЕКСТОВ: та же
# карта живёт в JS Mini App (miniapp/templates/subscriptions.html,
# _SHOPPING_CATEGORY_LABELS) — переименовал ярлык здесь → поправь там.
...
def category_label_ru(code: object) -> str:
    """Jinja-фильтр ``ru_category`` (регистрируется в registry env)."""
    s = str(code or "").strip()
    if not s:
        return s
    label = CATEGORY_LABELS_RU.get(s.lower())
    if label:
        return label
    cleaned = s.replace("_", " ").strip()
    if not cleaned:  # агент-ревьюер: «___» не превращать в невидимый заголовок
        return s

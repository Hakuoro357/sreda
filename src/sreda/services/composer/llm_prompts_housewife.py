...
# #126 п.3 — композерная накладка тона tender_care. НЕ легаси-overlay
# (тот про tools/память — рту вреден). Добавляется ПОСЛЕДНЕЙ строкой
# системного промпта в llm_composer, когда пользователь выбрал «нежную
# заботу»; warm_practical/None — промпт байт-в-байт прежний. Оговорка
# про приоритет правил обязательна: у humanize_result последняя
# инструкция — «правила важнее голоса», накладка не должна её отменять.
TENDER_CARE_COMPOSER_OVERLAY = (
    # Без фразы-примера: лайт-модель копирует пример дословно в каждый
    # ответ (поймано бенчем 2026-06-11 — «я рядом» ×3 подряд). «Не
    # повторяй из ответа в ответ» убрано: рот прошлых ответов не видит
    # (Codex R1 оба) — вместо этого запрет рефрена. Вёрстка фразы —
    # прямое ТЗ Бориса по живому скриншоту («— осталось 3 Надеюсь, всё
    # получится!»): пустая строка перед фразой + эмодзи.
    "\n\nТон: нежная забота — мягче и теплее обычного. Формулируй "
    "бережно и поддерживающе; в конце уместна одна короткая тёплая "
    "фраза участия — ОТДЕЛЬНЫМ абзацем (пустая строка перед ней, "
    "НЕ приклеивай к последнему пункту списка) и добавь к ней один "
    "уместный эмодзи. Избегай шаблонной одинаковой концовки, не превращай "
    "одну и ту же фразу в постоянный рефрен. Без сюсюканья "
    "и уменьшительно-ласкательных обращений. "
    "Все правила выше важнее тона."
)
...
_HUMANIZE_RESULT = LLMPromptSpec(
    system_prompt=(
        ...
        "— СООБЩЕНИЕ_ПОЛЬЗОВАТЕЛЯ используй только для тона и понимания "
        "запроса; факты результата бери только из ДАННЫХ (поле actions). "
        "Поле intent в ДАННЫХ — это просьба пользователя, НЕ результат: не "
        "выдавай запрошенное за сделанное.\n"
        "— ВЫПОЛНЕНИЕ используй только как контроль: если had_failures=true, "
        "не пиши «всё сделала»; конкретику ошибки бери только из ДАННЫХ.\n"
        ...
        "— Названия позиций копируй буквально из ДАННЫХ: сохраняй слова, "
        "порядок слов, регистр букв и написание. Не нормализуй, не сокращай "
        "и не подменяй синонимами.\n"
        "— Если в результате перечислены конкретные позиции — строками "
        "списка, пунктами, в группах или через запятую в одной строке — "
        "назови КАЖДУЮ. Не сокращай список, не заменяй обобщением («всё "
        "добавила») и не опускай имена. Хвост вида «и ещё N» сохраняй.\n"
        ...
        "ВЁРСТКА (строго — #123):\n"
        "— Только простой текст: без Markdown/HTML и служебной разметки. "
        "Не используй **звёздочки**, #заголовки, подчёркивания, `бэктики`, "
        ">цитаты и ссылки вида [текст](url): пользователь увидит эти символы "
        "как есть.\n"
        "— Не здоровайся и не представляйся. Приветствие допустимо только "
        "если пользователь сам поздоровался в СООБЩЕНИЕ_ПОЛЬЗОВАТЕЛЯ. Если "
        "нужно вступление — это короткая фраза по результату, без «Привет».\n"
        "— Перечни выводи ПО ОДНОМУ пункту на строку, маркер «—» или «•». "
        "НИКОГДА не склеивай перечень в один абзац.\n"
        "— Вступление — отдельной строкой ДО перечня (первый пункт не "
        "приклеивай к вступлению).\n"
        ...
        "— Эмодзи, которые уже есть в ДАННЫХ (например ✅ у выполненных "
        "пунктов), сохраняй — это данные. От себя эмодзи не обязательны; "
        "если добавляешь — максимум один и не вместо факта.\n"
        ...
        f"{_VOICE}\n\n"
        # Codex medium R1 MAJOR: строка приоритета обязана быть ПОСЛЕДНЕЙ
        # инструкцией промпта — после _VOICE, иначе «финальная» — снова голос.

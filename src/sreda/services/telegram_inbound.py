...
# ⚠️ Рамка честности (R1 sol MAJOR#1): сам `TelegramClient._post_request` и так делает до трёх
# попыток на timeout/5xx, т.е. окно дубля при потерянном ответе Telegram существует в проде
# ДО этой задачи и одинаково для всех тенантов. Здесь мы его НЕ расширяем (своей четвёртой
# попытки не добавляем) и НЕ сужаем — сужение ретраев клиента меняет доставку всем и идёт
# отдельной задачей.

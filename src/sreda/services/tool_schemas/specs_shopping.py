...
ADD_SHOPPING_ITEMS_SPEC = ToolSpec(
    ...
    mutex_notes=[
        "ТОЛЬКО для добавления новых пунктов. Отметить купленное — mark_shopping_bought, не add.",
        "Удаление/отмена — remove_shopping_items, не add с минусом.",
        "Правка существующей строки — update_shopping_item, не add+remove.",
    ],
    timeout_seconds=10,
...
MARK_SHOPPING_BOUGHT_SPEC = ToolSpec(
    ...
    mutex_notes=[
        "ТОЛЬКО для купленного (нужны item_ids из list_shopping). Удаление/отмена (не покупал) — remove_shopping_items.",
        "«Купил всё из списка» — сначала list_shopping, потом mark с полученными ids.",
        "Не add_shopping_items + mark одним планом — для добавить-и-сразу-отметить две отдельные операции подряд.",
    ],
    timeout_seconds=10,
    side_effect_class="transactional_write",
)
...
UPDATE_SHOPPING_ITEM_SPEC = ToolSpec(
    ...
    mutex_notes=[
        "Для правки ОДНОЙ строки. Bulk-категоризация — update_shopping_items_category.",
        "НЕ remove + add — это лишние LLM-итерации и латенси.",
    ],
    timeout_seconds=10,
    side_effect_class="transactional_write",
...
UPDATE_SHOPPING_ITEMS_CATEGORY_SPEC = ToolSpec(
    ...
    mutex_notes=[
        "Для BULK перекатегоризации. Одна строка — update_shopping_item.",
    ],
    timeout_seconds=10,
    side_effect_class="transactional_write",
)

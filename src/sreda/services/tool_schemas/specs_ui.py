...
REPLY_WITH_BUTTONS_SPEC = ToolSpec(
    ...
    description=(
        "Отправить ответ с 2-4 inline-кнопками-вариантами. ОБЯЗАТЕЛЬНО "
        "когда ответ содержит ВОПРОС к юзеру. Кнопки — короткие (≤20 "
        "симв) реплики, которые юзер сам мог бы написать. БЕЗ «Да/Нет» "
        "(vague) — конкретика: «Да, собери меню», «Не сейчас». БЕЗ "
        "пустых «Отмена»/«Назад» (Telegram сам даёт back-button). Нет "
        "вопроса — НЕ вызывай, отвечай обычным текстом."
    ),
    family="ui",
    effect="write",
    read_domains=[],

...
REPLY_WITH_BUTTONS_SPEC = ToolSpec(
    ...
    description=(
        "Отправить ответ юзеру с 2-4 inline-кнопками-вариантами. ИСПОЛЬЗУЙ "
        "ОБЯЗАТЕЛЬНО когда твой ответ содержит ВОПРОС к юзеру. Кнопки — "
        "короткие (≤20 симв) реплики, которые юзер сам мог бы написать. БЕЗ "
        "«Да/Нет» (vague) — всегда конкретика: «Да, собери меню», «Не "
        "сейчас». БЕЗ пустых «Отмена»/«Назад» (Telegram сам даёт "
        "back-button). Если вопроса в ответе НЕТ — НЕ вызывай, отвечай "
        "обычным текстом."
    ),
    family="ui",
    effect="write",

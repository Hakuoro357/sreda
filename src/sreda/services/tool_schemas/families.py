...
FAMILY_HEADERS: Final[Mapping[Family, FamilyHeader]] = {
    ...
    "tasks": FamilyHeader(
        ...
        anti_patterns=(
            ...
            "Граница с ЧЕК-ЛИСТАМИ: link_task_to_checklist связывает СУЩЕСТВУЮЩУЮ задачу с СУЩЕСТВУЮЩИМ чек-листом (логическая связь, оба остаются). move_task_to_checklist (в ЧЕК-ЛИСТАХ) превращает задачу в пункт чек-листа (задача сворачивается). Если юзер сказал «свяжи Y с чеклистом» — link_task_to_checklist. Если «перенеси Y в чеклист как пункт» — move_task_to_checklist.",
        ),
    ),

...
LOG_UNSUPPORTED_REQUEST_SPEC = ToolSpec(
    ...
    description=(
        ...
        "НЕ вызывай для того, что умеют существующие tools (напоминания / "
        "recall / save / web search), и НЕ как cop-out, когда просто надо "
        "подумать дольше. После лога ответь юзеру вежливо (кратко: чего не "
        "умею, и workaround если есть)."
    ),
    family="utility",
    effect="write",
    read_domains=[],

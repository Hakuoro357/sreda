...
LOG_UNSUPPORTED_REQUEST_SPEC = ToolSpec(
    ...
    description=(
        ...
        "feedback для product hotspot dashboard. ИСПОЛЬЗУЙ когда юзер "
        "просит что-то для чего у Среды нет skill/integration/tool/data: "
        "«закажи такси», «напиши код для X», «позвони в банк», "
        "«купи билеты». НЕ вызывай для того что РАЗ существующие "
        "tools (напоминания / recall / save / web search). НЕ вызывай "
        "как cop-out когда просто надо подумать дольше. После лога — "
        "ответь юзеру вежливо (объясни кратко что не умеешь, предложи "
        "workaround если есть). Возвращает ok:logged — user не видит."
    ),
    family="utility",
    effect="write",
    read_domains=[],

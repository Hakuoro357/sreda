...
_MEMORY = PendingReply(
    text=(
        "🧠 Память\n\n"
        "Главная фича: я запоминаю всё что важно, чтобы ты не "
        "держал(а) в голове.\n\n"
        ...
        "Когда подходит время — сама напомню в чате.\n\n"
        "📝 Дела и покупки — чек-листы без дат («сборы на дачу») и "
        "общий список покупок разбитый по категориям. Например: "
        "голосом «добавь молоко, хлеб, яблоки и пачку макарон» — "
        "раскидаю по разделам (молочка, хлеб, фрукты, бакалея); в "
        "магазине открыл — отметил купленное, оно уйдёт из списка.\n\n"
        "📖 Рецепты — сохраняю в книгу с КБЖУ, видишь калории и "
        "белки в каждой порции. Например: «у меня полкурицы, "
        "картошка, лук — что приготовить?» — подберу из книги по "
        "ингредиентам.\n\n"
        "👨‍👩‍👧 Семья — кто в семье, возраст, кружки, что любят и "
        "не любят. Например: знаю, что Маша не ест грибы — в меню "
        "на неделю не предложу, в покупки не попадут.\n\n"

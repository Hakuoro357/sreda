    ...
    *,
    ...
    Args:
        ...
        provider: имя провайдера для per-provider circuit breaker (#343).
            None → общий breaker-bucket (breaker всё равно работает).
    ...
    if cfg.enabled:
        if provider:
            ...
            if not breaker.allow(
                open_after=cfg.breaker_open_after, cooldown=cfg.breaker_cooldown
            ):
        ...
        if not bulkhead.try_acquire(cfg.max_concurrent):
            ...
            if breaker is not None:
                breaker.record_other_error()  # release any half-open probe slot
    ...
    try:
        ...
        # submit failed → never held the thread; release the permit now.
    ...
    try:
        ...
        executor.shutdown(wait=False, cancel_futures=True)
        if breaker is not None:
            breaker.record_timeout(open_after=cfg.breaker_open_after)
...
async def ainvoke_with_streaming_timeout(
    ...
    *,
    ...
    try:
        ...
        # Provider-side / cancellation errors: neutral to the breaker (no hung
        # thread to bound). The worker's own permit is released via its
        # done-callback whenever the thread finishes.
        if breaker is not None:
            breaker.record_other_error()
        raise

    # Stream completed (broke on "done") → the provider responded, not hung.
    if breaker is not None:
        breaker.record_success()
...
_OPENROUTER_EXTRA_BODY_BY_PROVIDER: dict[str, dict] = {
    # openrouter-gemini-2.5-flash-lite (пересказчик #232 + легаси chat/fact #197): пин на
    # провайдера google-vertex/eu СНЯТ 2026-07-01 (Борис). Причина: EU-Vertex — единственный
    # EU-регион у этой модели, его shared-пул (не-BYOK) деградировал → 429 "rate-limited
    # upstream" (аптайм 30мин ~60%; тот же корень, что #257 «gemini-lite зависал»). only +
    # allow_fallbacks=False не давал уйти на живой эндпоинт → пересказчик падал. Без пина
    # OpenRouter сам роутит на здоровый провайдер. ТРЕЙД-ОФ: ПД выжимок могут обрабатываться
    # вне EU (data-residency НЕ гарантирован). Residency-сохраняющая альтернатива — BYOK
    # Vertex-EU ключ (своя квота) — отложена.
...
def _build_request_timeout(settings: Settings) -> httpx.Timeout:

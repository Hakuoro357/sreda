...
- Заземление реплики на результат: **#393** (класс #376 «сделала, но сказала не то»). PATH-AGNOSTIC
  (легаси И unified, гейт `react_post_tool_report_enabled`, НЕ `_unified_execute_for`): (1) `_ground393` —
  хвост-директива узла `chat` со сводкой результата успешного write (чтобы голос назвал имена тепло, #121);
  (2) страховка в финализации `handle_turn` — если реплика не называет результат, детерминированная
  заземлённая подмена. Ядро — `react_result_report.py` (allowlist заземляемых действий + проверка эффекта +
  санитизация имён; прецедент детерминированной подмены — `_declined_reply` #321).

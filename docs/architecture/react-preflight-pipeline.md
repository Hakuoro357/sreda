...
- **write двухъярусный (B2):** сигнал+продуктивный домен → инструмент напрямую; БЕЗ сигнала → все write-инструменты
  ...
  превью без чтения БД; инвариант «нет молчаливой записи»). Исключение #389/#392: аддитивные
  `add_shopping_items` (#389) и `add_checklist_items` (#392) (`_UNIFIED_AUTOEXEC_WRITE_TOOLS` +
  owner-allowlist, import-time гвард реестра) биндятся прямым, если роутер НЕ дал КОНКУРИРУЮЩЕГО
  write-домена (aw ⊆ доменов инструмента; «добавь в список дел купить молоко» → aw={checklists} →
  для add_shopping_items кандидат+confirm, примиряющий расхождение «роутер vs модель»; симметрично
  add_checklist_items при aw={shopping}) — прод-трассы/владелец показали чистое трение confirm'а на
  продолжении диктовки порциями (покупки «1 литр молока»/«ещё добавь соль» #389; чек-листы «Ещё
  добавь уголь» #392 — оба resolved yes); аддитивно/видимо/обратимо, ПОШТУЧНО с owner-«да»; фикс НЕ
  расширяет autoexec на деструктив (remove_*/clear_*/delete_*/archive_* — в общем двухъярусном
  контуре, candidate-confirm при aw=∅ — пред-существующее поведение, не гарантия исключения);

...
- **write двухъярусный (B2):** сигнал+продуктивный домен → инструмент напрямую; БЕЗ сигнала → все write-инструменты
  ...
  (`_UNIFIED_AUTOEXEC_WRITE_TOOLS`) биндится прямым ВСЕГДА — прод-трассы показали чистое трение confirm'а на
  продолжении диктовки покупок («1 литр молока», «ещё добавь соль» → оба yes); аддитивно/видимо/обратимо,
  деструктив shopping (remove_*/clear_*) остаётся под confirm;

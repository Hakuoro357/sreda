...
`react_loop.handle_turn` на свежей sqlite. Судим по **ИНВАРИАНТАМ СОСТОЯНИЯ**
(БД + квитанции инструментов + текст реплики + потолки), а не «ответ похож».
...
| `390_show_lists_tech_leak` | #390 открыт | сырой вывод `get_checklist(overview)` = англ. «pending/done/total» | **xfail** (красный) |

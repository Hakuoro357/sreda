...
def test_confirm_reply_is_noise():
    ...
    for t in ("", "ок", "окей", "конечно", "давай", "ну давай", "удали", "удалите", "ок удали",
              ...
              "ой", "вот", "эх"):  # #362 R2 (Codex sol+terra MINOR): нейтральный дискурс-филлер = noise
...
def test_confirm_decline_signal():
    ...
    # отказ → True: exact, фразы, отрицаемая команда, ведущий филлер, ГОЛЫЙ/ЧИСЛОВОЙ хвост (leak-proof R5)
    for t in ("нет", "отмена", "неа", "не надо", "не нужно", "не согласен", "не согласна", "не согласны",
              ...
              "нет, 16", "нет 16", "нет, шестнадцать", "ну нет, 16", "да нет",
              # #362 R2 (Codex sol+terra MAJOR): разговорное «нет» + голые негативные императивы
              "не-а", "нее", "не а", "неее", "ну нее", "да нее", "не делай", "не трогай", "не выполняй"):
        assert d(t), f"{t!r} — явный отказ, должен быть decline (честная #321)"
    # НЕ чистый отказ → False (редирект). R5 leak-proof (Codex sol, повторные dual-intent-утечки): ЛЮБОЕ
    # СЛОВО в хвосте отказа → редирект (никогда не глотаем команду/запись/ссылку — «нет, по задаче 5»/
    # «отмена, задача 5»/«не удали задачу 5» → свежий ход, БЕЗОПАСНО). Плюс запись/новый запрос/да/эхо.
    for t in ("нет, сахар 16", "нет, запиши 16", "нет, добавь задачу 5", "нет, запиши температуру высокую",
              "отмени задачу 5", "нет, отмени задачу 5", "нет, удали задачу 3", "нет, отменить пункт 2",
              "нет, по задаче 5", "отмена, задача 5", "не удали задачу 5",
              # #362 R2 (Codex sol MAJOR): рыхлый «ни..что» БОЛЬШЕ не глотает команду/показатель
              "ни что запиши 16", "ни что сахар 16", "не делай задачу 5",
              # #362 R2 (Codex terra MAJOR): СТРУКТУРНЫЙ показатель (≥2 числа) после «нет» → НЕ отказ (не терять)
              "нет, 16.2", "нет, 120/80", "нет 16 2", "нет сто двадцать на восемьдесят",
              # #362 R3-R5 (Codex sol+terra MAJOR): число в ЧИСЛОВОЙ НОТАЦИИ (единица/сравнение/знак/десятичная) → показатель, не отказ
              "нет, 98%", "нет, 36°", "нет, <5", "нет, >10", "нет, ≤5", "нет, ≥10", "нет, ±2",
              "нет, -2", "нет, −2", "нет, .5", "нет, 36℃", "нет, <=5", "нет, >=10",
              SUGAR, "температура высокая", "покажи покупки", "да", "ок", "удали"):
        assert not d(t), f"{t!r} — НЕ чистый отказ (dual-intent/команда/показатель/да/эхо) → редирект"


def test_struct_num_regex_no_false_positive_362():
    """#362 R5-корень (решение оркестратора: опция A + сужение): `_STRUCT_NUM_RE` — числовая нотация, а НЕ
    голый `\\d[^\\w\\s]`. Ключевой риск (владелец): широкий регекс ложно ловил бы ординалы/списки как
    «показатель». Пин: регекс МАТЧИТ все клин-формы и НЕ матчит ординалы/списки/голые числа.
    NB: генуинная отмена со СЛОВОМ в хвосте («нет, хватит на 100%»/«нет, 3-й раз уже») и так уходит в
    редирект по leak-proof (слово в хвосте), НЕ через этот регекс — удаление снимается, потери нет."""
    from sreda.runtime.react_signals import _STRUCT_NUM_RE as rx
    # МАТЧИТ (структурный показатель → редирект): единица/сравнение/знак/десятичная/отношение
    for t in ("98%", "36°", "36℃", "5.5", "16.2", "36,6", ".5", "120/80", "<5", "<=5", "≤5",
              ">10", ">=10", "≥10", "-2", "−2", "±2", "+5"):
        assert rx.search(t), f"{t!r} — числовая нотация, регекс ДОЛЖЕН матчить (→ редирект показателя)"
    # НЕ матчит (ложные срабатывания — ординалы/списки/голое число/пунктуация без числа): → decline/бэйл
    for t in ("3-й", "5-м", "10-го", "2-я", "1)", "2.", "16", "шестнадцать", "нет", "!!!", ")", "договорились"):
        assert not rx.search(t), f"{t!r} — НЕ числовая нотация, регекс НЕ должен матчить (ложное срабатывание)"
...
def test_should_redirect_declarative_record_362():
    ...
    for t in (SUGAR, "сахар 16 и 2", "16.2", "давление 120 на 80",
              ...
              "ни что запиши 16",  # #362 R2 (Codex sol MAJOR): показатель после рыхлого «ни..что» не теряется
              "нет, 16.2", "нет, 120/80",  # #362 R2 (Codex terra MAJOR): структурный показатель после «нет» не теряется
              "нет, 98%", "нет, 36°", "нет, ≤5", "нет, ≥10"):  # #362 R3/R4 (Codex sol+terra MAJOR): показатель с единицей/сравнением не теряется
        assert f(t, is_confirm_pause=True), f"confirm: запись {t!r} должна redirect (свежий ход)"
    # честная отмена #321 — НЕ redirect (уходит в resume→honest «Отменила»); ГОЛЫЙ/числовой отказ
    for t in ("нет", "отмена", "не надо", "нет, 16", "не удали", "нет, шестнадцать", "ну нет, 16",
              # #362 R2 (Codex sol+terra): разговорное «нет» + голый негативный императив + нейтр. филлер
              "не-а", "нее", "ну нее", "не делай", "ой", "вот", "эх"):
        assert not f(t, is_confirm_pause=True), f"confirm: {t!r} — честная отмена/noise, НЕ redirect"
    # R5 leak-proof: отказ со СЛОВОМ в хвосте → редирект (безопасно, команда/ссылка не теряется)
    for t in ("нет, отмени задачу 5", "нет, по задаче 5", "отмена, задача 5"):
        assert f(t, is_confirm_pause=True), f"confirm: {t!r} — слово в хвосте отказа → редирект"
...
@pytest.mark.parametrize("refusal", [
    "нет, 16", "не надо",                              # R2 формы (голый/числовой хвост)
    "не удаляй", "не согласна", "ну нет, 16",          # R4 (оба Codex MAJOR)
])
def test_genuine_decline_with_number_tail_still_honest_362(install, monkeypatch, tmp_path, refusal):
    """#362 R2/R4/R5 (оба Codex MAJOR): ГОЛЫЙ/числовой отказ («нет, 16»/«не удаляй»/«не согласна»/
    «ну нет, 16») → честная детерминированная «Отменила», мутация НЕ исполнена. Протокол-семантика:
    явный отказ (confirm_decline_signal → classify negate) идёт в resume, не редирект. R5 leak-proof:
    отказ со СЛОВОМ в хвосте («нет, по задаче 5»/«нет, сахар 16») → редирект (проверено в юнит-тестах),
    чтобы НИКОГДА не глотать команду/показатель после отказа."""

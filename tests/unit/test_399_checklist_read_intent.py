...
SHOW_CASES = [
    ...
    "покажи список идей",
    # CR R3 sol MINOR: настоящий запрос, потерянный сужением до императива — возвращён
    "есть ли у меня список кино",
    # CR R4 sol MAJOR: имя списка «слов» само попадало в мета-словарь (`слов\w*`) и
    # глушило ОСНОВНУЮ форму — т.е. правка вернула бы исходный баг на реальной фразе
    "покажи, пожалуйста, список слов",
]
...
NOT_A_REQUEST_CASES = [
    ...
    "что за список кино",
    # CR R3 sol MAJOR: мета-рамка ПОСЛЕ маркера
    "покажи список кино — это команда?",
    "скажи, что значит список кино",
    # CR R4 sol MAJOR: мета дальше двух слов (окно {0,2} пропускало)
    "покажи список кино — а это разве команда?",
]


# ОТЛОЖЕННЫЙ MINOR (sol R2, подтверждён на R4): составная фраза с ЦИТИРУЕМЫМ маркером в
# другой клаузе подавляется целиком. Ошибка в БЕЗОПАСНУЮ сторону (нет сигнала → текущее
# поведение web-only, не утечка); разделение клауз — заметная новая машинерия ради
# искусственного кейса. Тест фиксирует ФАКТ, чтобы поведение не менялось молча.
DEFERRED_OVERSUPPRESSED = [
    "покажи список кино, а фраза «открой список книг» звучит странно",
]
...
class TestPolicySeam:
    ...
    @pytest.mark.parametrize("text", DEFERRED_OVERSUPPRESSED)
    def test_known_oversuppression_is_recorded_not_silent(self, text):
        """Известное ИЗЛИШНЕЕ подавление — отложенный MINOR, зафиксирован намеренно.
        Покраснеет, если кто-то починит (тогда ожидание правим осознанно) или если
        поведение уедет в опасную сторону."""
        assert checklist_read_intent(text) == frozenset()

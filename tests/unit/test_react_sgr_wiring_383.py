...
# Ре-база 2026-07-20 (merge #362 в ветку): #362 R5 осознанно изменил _unified_availability_directive
# (availability-хвост) — «инструмент ТЕКУЩЕГО действия … реальную отмену» + клауза «служебная пометка
# о СНЯТИИ прошлого вызова — НЕ отмена». Снапшот синхронизирован с новым текстом хвоста.
_GOLDEN_OFF_HUMAN = (
    ...
    "Если инструмент ТЕКУЩЕГО действия вернул реальную отмену или «не делаю» — так и сообщи "
    "(«отменила, ничего не делаю»), и ОСТАНОВИСЬ: НЕ переспрашивай и не предлагай сделать это "
    "снова. НО служебная пометка о СНЯТИИ прошлого незавершённого вызова («вызов инструмента "
    "отменён … не считать выполненным») — это НЕ отмена текущего запроса: НЕ сообщай о ней как "
    "об отмене, просто обработай текущий запрос по существу. Не отвечай мимо запроса "

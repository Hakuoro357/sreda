...
@pytest.mark.parametrize("text", [
    ...
    # R5 (CodexH/субагент): атрибуция ПОСЛЕ команды + страдательная/иная форма рамки
    "добавь молоко, сказал он",
    "удали задачу, попросила мама",
    "в инструкции написано добавь молоко",
    "там написано удали задачу",
    "инструкция гласит: добавь молоко",
    "цитирую: добавь молоко",
])
...
@pytest.mark.parametrize("text", [
    ...
    "добавь молоко, пожалуйста",       # запятая + не-рамка → команда (не атрибуция)
    "добавь молоко, а потом позвони",  # запятая + союз → команда
])
...
@pytest.mark.parametrize("text", [
    ...
    # R5 (CodexH/CodexM/субагент): атрибуция-после, «по словам», голый стем KIDS
    "меня зовут Таня, сказала она",
    "меня зовут Таня, представилась она",
    "у меня двое детей, сказала сестра",
    "по словам мамы, меня зовут Таня",
    "со слов сестры, у меня двое детей",
    "у меня 2 детских сада",
])

...
#138 Ф2: ``OutboxDeliveryWorker`` больше НЕ принимает сессию — сам открывает
seam-скоупы (``privileged_session("queue-dispatch")`` для кросс-тенант скана
очереди + ``tenant_session(tenant_id)`` на строку). Тесты #1/#3 гоняют полный
``process_pending_messages`` → через фикстуру ``worker_db`` (коммитящая файловая
SQLite + шов ``_factory_for`` привязан к ней), чтобы сессии воркера и сессия
теста делили одну БД. Сессии РАЗНЫЕ (identity map не общий), поэтому статус
outbox-строки (мутация чужой сессией) перечитываем через ``worker_db.expire_all()``
+ re-fetch. Тест #2 (hybrid-UX) воркер не трогает, но делит общий сид → тоже на
``worker_db`` (``ActionRuntimeService`` работает на переданной сессии как раньше).
"""
...
def _bootstrap(session):
    """Сид семьи на переданной сессии ``worker_db``.

    #138 Ф2: воркер открывает СВОИ seam-сессии (через ``_factory_for``, который
    ``worker_db`` патчит на тестовую фабрику), поэтому сид коммитим на общую с
    воркером БД ДО его прогона. Возвращаем ту же сессию для сида/ассертов теста.
    """
...
def test_acceptance_quiet_hours_defers_and_wakes(worker_db):
    session = _bootstrap(worker_db)
    ...
    # #138: воркер мутировал строку СВОЕЙ сессией → сбрасываем засиженное
    # состояние в сессии теста перед чтением статуса/scheduled_at.
    session.expire_all()
...
def test_acceptance_hybrid_ux_confirm_flow(worker_db):
    session = _bootstrap(worker_db)
    telegram = FakeTelegram()
    service = ActionRuntimeService(session, telegram_client=telegram)
...
def test_acceptance_per_skill_mute_and_urgent(worker_db):
    session = _bootstrap(worker_db)
    telegram = FakeTelegram()
    service = ActionRuntimeService(session, telegram_client=telegram)
    ...
    # #138: статус мутирован сессией воркера → expire перед чтением.
    session.expire_all()

...
def test_postformat_rich_normalizes_double_asterisk():
    ...
    # нормализация продолжает работать в ТЕКСТЕ рядом с кодом, а не только вместо него
    assert react_loop._postformat("**Итог:** см. `a**b`", rich=True) == "*Итог:* см. `a**b`"


@pytest.mark.parametrize("src", [
    "2**3 + 4**5",                            # арифметика, а не разметка
    "`print(2**3, 4**5)`",                    # код в бэктиках
    "путь **/*.py и **/*.md",                 # глоб-шаблоны в прозе
    '`glob = "**/*.py"; other = "**/*.md"`',  # две глоб-маски в кавычках внутри кода (R3 sol)
    '`foo **bar** baz`',                      # жирный ВНУТРИ inline-кода — это содержание
    "```\nx = '**bold**'\ny = '**also**'\n```",  # fenced-блок целиком неприкосновенен

...
def test_hint_requires_end_clarification_333():
    """Решение владельца 2026-07-10 «конец обязателен к уточнению» с #350 живёт
    в МЕХАНИЗМЕ (диспатч-гейт recurrence_end_not_specified), не в хинте: хинт
    «сперва спроси» оставлял серию без слот-исхода → цепочки #338/#349 не
    сцеплялись (прод 2026-07-11 13:30). Хинт теперь велит ЗВАТЬ инструмент
    сразу и не спрашивать до вызова."""
    hint = _recurrence_hint("поставь напоминание каждый час пить воду")
    assert hint is not None
    assert "ВЫЗЫВАЙ" in hint
    assert "НЕ спрашивай ДО вызова" in hint

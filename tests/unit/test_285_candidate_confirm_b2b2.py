...
def test_account_deletion_signal_precision():
    ...
    for t in ("удали мой аккаунт", "хочу удалить свою учётную запись", "снеси мой профиль",
              "удали мою учётку", "уничтожь мой аккаунт", "хочу удалить свой профиль"):
        assert a(t) is True, t
    # негация/вопрос/цитата/мета/чужой (B2 R2-R3 Codex+субагент) + не-аккаунт; притяжательное обязательно
    for t in ("удали задачу", "добавь молоко", "как дела?", "удали напоминание про врача",
              ...
              "надо удалить аккаунт в инстаграме", "надо удалить аккаунт"):  # R3: чужой/безлич.
